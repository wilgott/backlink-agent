...
const SIGNUP_URL = 'https://findly.tools/sign-in?from=submit';
const SUBMIT_URL = 'https://findly.tools/submit';
...
async function main() {
  ...
  const userDataDir = path.resolve(input.paths.user_data_dir || './state/profiles/findly');
  ...
  try {
    ...
    if (/verify your email|check your (inbox|email)|confirmation (link|email)|we sent (you )?a|enter (the )?code/i.test(after)) {
      return finish('NEEDS_EMAIL_VERIFICATION', {
        reason: 'Findly requires email verification',
        email_verification: { expected_sender_pattern: 'findly', expected_subject_pattern: 'verif|confirm|code', max_wait_minutes: 30 },
      });
    }
    ...
    if (/sign[- ]?in|log in|create (your )?account/i.test(page.url() + ' ' + sub.slice(0, 200))) {
      return finish('NEEDS_EMAIL_VERIFICATION', {
        reason: 'Findly still on auth after signup (likely email verify)',
        email_verification: { expected_sender_pattern: 'findly', expected_subject_pattern: 'verif|confirm', max_wait_minutes: 30 },
      });
    }
