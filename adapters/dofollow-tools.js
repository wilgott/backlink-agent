// Adapter: Dofollow.tools (https://dofollow.tools/submit)
...
//   node adapters/dofollow-tools.js <input.json> <output.json>
...
const DEFAULT_URL = 'https://dofollow.tools/submit';
...
async function main() {
  ...
  try {
    ...
    if (!(await submit.count())) {
      return finish('ERROR', { reason: `no submit button on step 2. body=${body2.slice(0, 250)}` });
    }
    ...
    if (/sign in|log in|create (an )?account/i.test(after) && /sign-in|login|signup/i.test(afterUrl)) {
      ...
      return finish('NEEDS_EMAIL_VERIFICATION', {
        reason: 'Submit for Free redirected to OTP login. Filled ' + email + '. Send Code gated by Cloudflare Turnstile (waited, no auto-token).',
        email_verification: { expected_sender_pattern: 'dofollow', expected_subject_pattern: 'verif|confirm|code|account', max_wait_minutes: 30 },
