...
const SIGNUP_URL = 'https://smollaunch.com/signup?intent=submit_product&return_to=%2Fdashboard%2Fproducts%2Fnew';
const SUBMIT_URL = 'https://smollaunch.com/dashboard/products/new';
...
async function main() {
  ...
  const userDataDir = path.resolve(input.paths.user_data_dir || './state/profiles/smol-launch');
  ...
  try {
    ...
    if (/verify your email|check your (inbox|email)|confirmation (link|email)|we sent (you )?a|enter (the )?code/i.test(after)) {
      return finish('NEEDS_EMAIL_VERIFICATION', {
        reason: 'Smol Launch requires email verification',
        email_verification: { expected_sender_pattern: 'smol-launch', expected_subject_pattern: 'verif|confirm|code', max_wait_minutes: 30 },
      });
    }
    ...
    if (/sign[- ]?in|log in|create (your )?account/i.test(page.url() + ' ' + sub.slice(0, 200))) {
      return finish('NEEDS_EMAIL_VERIFICATION', {
        reason: 'Smol Launch still on auth after signup (likely email verify)',
        email_verification: { expected_sender_pattern: 'smol-launch', expected_subject_pattern: 'verif|confirm', max_wait_minutes: 30 },
      });
    }
